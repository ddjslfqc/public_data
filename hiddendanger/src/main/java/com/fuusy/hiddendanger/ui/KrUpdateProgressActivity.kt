...
import com.fuusy.hiddendanger.databinding.ActivityKrUpdateProgressBinding
...
class KrUpdateProgressActivity : AppCompatActivity() {

    private lateinit var binding: ActivityKrUpdateProgressBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (item == null || !KrProgressHelper.canUpdateProgress(item)) {
            Toast.makeText(this, "当前无法更新进度", Toast.LENGTH_SHORT).show()
            finish()
            return
        }
        krItem = item

        binding = ActivityKrUpdateProgressBinding.inflate(layoutInflater)
    ...
    private fun setupAttachments() {
        ...
        binding.rvAttachments.apply {
            layoutManager = GridLayoutManager(this@KrUpdateProgressActivity, 4)
                ...
                when (selected.value) {
                    ...
                    "camera" -> {
                        cameraResultLauncher.launch(
                            Intent(this@KrUpdateProgressActivity, CameraActivity::class.java)
                        )
                    }
                }
                dialog.dismiss()
    ...
    companion object {
        fun start(context: Context, item: GoalKrItem) {
            context.startActivity(
                Intent(context, KrUpdateProgressActivity::class.java).apply {
