...
open class BaseRepository {
    ...
    suspend fun <T : Any> executeResp(
        ...
        try {
            ...
            Log.d(TAG, "executeResp: errorCode = ${baseResp.errorCode}")
            Log.d(TAG, "executeResp: data = ${baseResp.data}")
            Log.d(TAG, "executeResp: data type = ${baseResp.data?.javaClass?.simpleName}")
            
            if (baseResp.errorCode == 200) {
                if (baseResp.data == null || baseResp.data is List<*> && (baseResp.data as List<*>).size == 0) {
                    Log.d(TAG, "executeResp: 数据为空，设置 STATE_EMPTY")
                    baseResp.dataState = DataState.STATE_EMPTY
                } else {
                    Log.d(TAG, "executeResp: 数据不为空，设置 STATE_SUCCESS")
                    baseResp.dataState = DataState.STATE_SUCCESS
                }
            } else {
